...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (type) {
      ...
      case 'leave':
        return handleLeave(room, userId);
      default:
        return new Response(
...
function handleOffer(room: Room, userId: string, payload: any): Response {
  ...
  console.log(`[WebRTC Signaling] Offer received from ${userId} in room ${roomId}`);
 ...
 */
function handleLeave(room: Room, userId: string): Response {
  // Remove participant
