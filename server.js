...
// ── Rate limiter (server-level, bypasses middleware) ────────────
const RATE_LIMIT_MAX = 10;       // 10 requests per minute for /api/*
  ...
  if (redisUrl && redisToken) {
    try {
      const res = await fetch(`${redisUrl}/pipeline`, {
        ...
        body: JSON.stringify([
          ["INCR", redisKey],
          ["EXPIRE", redisKey, Math.ceil(RATE_LIMIT_WINDOW_MS / 1000)],
        ]),
      });
      ...
      if (count > RATE_LIMIT_MAX) {
        return { allowed: false, remaining: 0, resetTime: now + RATE_LIMIT_WINDOW_MS };
      }
      return { allowed: true, remaining: RATE_LIMIT_MAX - count, resetTime: now + RATE_LIMIT_WINDOW_MS };
    } catch (err) {
      console.warn("[rateLimit] Redis error, using memory:", err.message);
    }
  }

  // In-memory fallback
  const entry = rlMemory.get(identifier);
  if (entry && entry.resetTime < now) rlMemory.delete(identifier);
  const current = rlMemory.get(identifier) || { count: 0, resetTime: now + RATE_LIMIT_WINDOW_MS };
  if (current.count >= RATE_LIMIT_MAX) {
    return { allowed: false, remaining: 0, resetTime: current.resetTime };
  }
  current.count++;
  rlMemory.set(identifier, current);
...
function boot() {
  const httpServer = createServer((req, res) => {
    ...
    if (url.pathname.startsWith("/api/")) {
      const identifier = req.headers["x-forwarded-for"] || req.headers["x-real-ip"] || "anonymous";
      checkRateLimit(identifier).then((rlResult) => {
        if (!rlResult.allowed) {
          console.log(`[rateLimit] 429 for ${identifier} on ${url.pathname}`);
          res.writeHead(429, {
            "Content-Type": "application/json",
            "X-RateLimit-Limit": String(RATE_LIMIT_MAX),
            "X-RateLimit-Remaining": "0",
            "X-RateLimit-Reset": String(rlResult.resetTime),
          });
