...
export default function CreditMeter({
  ...
  // Update ref when balance changes
  useEffect(() => {
    balanceRef.current = balance;
  }, [balance]);

  // Call callbacks when balance changes
  useEffect(() => {
    if (balance !== prevBalanceRef.current) {
      if (balance < prevBalanceRef.current) {
        onCreditDeducted?.(balance);
      }
      if (balance <= 0 && prevBalanceRef.current > 0) {
        onExhausted?.();
      }
      prevBalanceRef.current = balance;
    }
  }, [balance, onCreditDeducted, onExhausted]);
